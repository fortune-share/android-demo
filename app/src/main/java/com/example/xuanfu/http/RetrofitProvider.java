...
import com.example.xuanfu.AppConfig;
import com.example.xuanfu.Application;
import com.example.xuanfu.http.cookie.CookieJarImpl;
import com.example.xuanfu.http.cookie.store.PersistentCookieStore;
...
//    }
    ...
    public static RetrofitProvider getInstance(boolean isNeedAuthorization) {
        return new RetrofitProvider(isNeedAuthorization);
    }
...
//    }
    ...
    private RetrofitProvider(boolean isNeedAuthorization) {
        ...
        okHttpClient = new OkHttpClient.Builder()
                .cookieJar(new CookieJarImpl(new PersistentCookieStore(mContext)))
                .cache(cache)
                .addInterceptor(new MyInterceptor(isNeedAuthorization))
//                .addInterceptor(new CacheInterceptor(mContext))
                .addInterceptor(new LoggingInterceptor())
                .connectTimeout(DEFAULT_TIMEOUT, TimeUnit.SECONDS)
                .writeTimeout(DEFAULT_TIMEOUT, TimeUnit.SECONDS)
